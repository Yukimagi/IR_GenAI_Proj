...
async def analyze_and_store_sentiments():
    # 從 sport_news 表中提取所有數據 (包括 sport_news 和 sport_news_API)
    news_data_sport = await fetch_all_data("sport_news")
    news_data_api = await fetch_all_data("sport_news_API")

    # 合併來自兩個表的數據
    news_data = news_data_sport + news_data_api

    if not news_data:
        print("No news data found in sport_news.")
        return
    ...
    for news in news_data:
        try:
            ...
            # # 檢查該 news_id 是否已存在於 sport_news_sentiment 表中
            # existing_sentiment = supabase.from_("sport_news_sentiment").select("id").eq("news_id", news["id"]).execute()
            
            # # 如果該新聞已存在，則刪除舊記錄
            # if existing_sentiment.data:
            #     supabase.from_("sport_news_sentiment").delete().eq("news_id", news["id"]).execute()
            #     print(f"Deleted old sentiment analysis for news ID: {news['id']}")

            # 插入新的情緒分析結果
            insert_response = supabase.from_("sport_news_sentiment").insert({
